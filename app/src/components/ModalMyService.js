...
const ModalMyService = ({ show, onClose, children, serviceImg, serviceName, serviceDesc, serviceCategory, servicePrice, serviceBonusToken, serviceFees, serviceExtra, serviceRate }) => {
    ...
    const addService = async (e) => {
        ...
        formData.append('services', serviceImg)
        formData.append('name', serviceName)
        formData.append('description', serviceDesc)
        formData.append('category', serviceCategory)
        formData.append('price', servicePrice)
        formData.append('bonusToken', serviceBonusToken ? serviceBonusToken : 0)
        formData.append('fees', serviceFees ? serviceFees : 0)
        formData.append('extra', serviceExtra ? serviceExtra : 0)
        formData.append('rate', serviceRate ? serviceRate : 0)

        await axios.post(`/api/service/addservice/${ user.id }`, formData, {
            headers: {
                ...
                <div className={ modalClasses }>
                    ...
                    <div className="mt-4">{ children }
                        <div className='flex justify-end text-white mt-4'>
                            <button onClick={ e => addService(e) } className='bg-emerald-500 hover:bg-red-700 py-2 px-4 rounded'>Add</button>
                        </div>
                    </div>
                </div>
            </div>
