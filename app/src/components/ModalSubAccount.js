...
const ModalSubAccount = ({ show, onClose, children, name, email, mobilenum, password, confirmpw }) => {
    ...
    const createSubAccount = async (e) => {
        ...
        formData.append('refCode', user.refnum)

        await axios.post(`/api/user/createsubaccount/${ user.id }`, formData, {
            headers: {
            ...
            <div className={ modalClasses }>
                ...
                <div className="mt-4">{ children }
                    <div className='flex justify-end text-white mt-4'>
                        <button onClick={ e => createSubAccount(e) } className='bg-emerald-500 hover:bg-red-700 py-2 px-4 rounded'>Create</button>
                    </div>
                </div>
            </div>
        </div>
...
export default ModalSubAccount
