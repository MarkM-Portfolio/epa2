...
const ModalDelegate = ({ show, onClose, children, otp }) => {
    ...
    const setDelegatePin = async (e) => {
        e.preventDefault()

        const CancelToken = axios.CancelToken
        const source = CancelToken.source()

        await axios.put(`/api/user/delegate-pin/${ user.id }`, { otp: otp }, {
            headers: {
            ...
            <div className={ modalClasses }>
                ...
                <div className="mt-4">{ children }
                    <div className='flex justify-center text-white mt-4'>
                        <button onClick={ e => setDelegatePin(e) } className='bg-blue-500 hover:bg-red-700 py-2 px-4 rounded'>Set Pin</button>
                    </div>
                </div>
            </div>
        </div>
