...
import ModalMyService from '../../ModalMyService'
...
const MyService = () => {
    ...
    const [ serviceBonusToken, setServiceBonusToken ] = useState('')
    const [ serviceFees, setServiceFees ] = useState('')
    const [ serviceExtra, setServiceExtra ] = useState('')
    const [ serviceRate, setServiceRate ] = useState('hour')
    const [ highToken, setHighToken ] = useState('')
    const [ lowToken, setLowToken ] = useState('')
    const [ serviceImgEdit, setServiceImgEdit ] = useState(false)
    ...
    useEffect(() => {
        if (newServiceImg) {
            newServiceImg.destroy()
            setNewServiceImg(null)
        }
    }, [])
        ...
        if (file) {
            ...
            reader.onload = function (e) {
                if (croppieRef.current) {
                    ...
                    setNewServiceImg(croppie)
                    setServiceImgEdit(true)
    ...
    const handleFileSet = async () => {
        ...
        if (newServiceImg) {
            croppedImageBlob = await newServiceImg.result({
                type: 'blob',
                size: 'viewport',
        ...
        setServiceImg(croppedImageBlob)
        setServiceImgEdit(false)
        newServiceImg.destroy()
    }
        ...
        <>
            <div className='font-montserrat lg:hidden'>
                ...
                <div className='flex justify-center gap-2 text-3xl -mt-7'>
                    <GrServices className='fill-orange-500' />
                    <h1 className='text-2xl text-center font-semibold mb-10 pl-1'>My Services</h1>
                </div>
                <hr />
                ...
                <div className='mt-2 px-6'>
                    { currentUser && currentUser._id && services.length ? 
                        <div className='mt-4'>
                            ...
                            <div className='flex flex-wrap gap-2 mb-4'>
                                { services.map(item => (
                                    <div className="flex grow items-end mt-4" key={ item._id }>
                                        <div className='border h-30 w-full rounded-lg shadow-md'>
                                            <NavLink to={ "/editservice" } state={ item }>
                                                <div className='items-center'>
                                                    <img className='onject-contain w-full mb-2' src={ window.location.origin + '/public/services/' + item.image } />
                                                </div>
                                ...
                                <div className="px-6 py-4" key={ item._id }>
                                    <div className="relative flex w-full max-w-xs flex-col overflow-hidden rounded-lg border border-gray-100 bg-white shadow-md">
                                        ...
                                        <img className="object-cover" 
                                            src={ window.location.origin + '/public/services/' + item.image }
                                            alt='service image'
                                        />
                                        </a>
                    ...
                    <ModalMyService serviceImg={ serviceImg } serviceName={ serviceName } serviceDesc={ serviceDesc } serviceCategory={ serviceCategory } servicePrice={ servicePrice } serviceBonusToken={ serviceBonusToken } serviceFees={ serviceFees } serviceExtra={ serviceExtra } serviceRate={ serviceRate } show={ show } onClose={ closeModal }>
                        ...
                        <div className="flex justify-center pt-2">
                            { !serviceImgEdit ?
                                ...
                                <img className={ `h-20 ${ innerWidth >=400 ? 'w-[118px]' : 'w-[92px]' }` }
                                    src={ newServiceImg } alt="new_service_img" />
                                :
                                <img className={ `h-20 ${ innerWidth >=400 ? 'w-[118px]' : 'w-[92px]' }` }
                                ...
                                newServiceImg ? '' :
                                <img className={ `h-20 ${ innerWidth >=400 ? 'w-[118px]' : 'w-[92px]' }` }
                                    src={ newServiceImg } alt="new_service_img_edit" />
                            }
                        </div>
