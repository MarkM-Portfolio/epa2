...
const ProductPage = () => {
    ...
    const addRatings = async (e) => {
        ...
        await axios.put(`/api/product/setratings/${ user.id }`, formData, {
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
                'X-Api-Key': process.env.API_KEY
    ...
    const addFavorites = async (e) => {
        ...
        await axios.put(`/api/product/setfavorites/${ user.id }`, formData, {
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
                'X-Api-Key': process.env.API_KEY
            ...
            <div className='lg:hidden font-montserrat'>
                ...
                <div className='mt-10 flex justify-center'>
                    <div className={ `h-30 ${ innerWidth >=400 ? 'w-[178px]' : 'w-[152px]' } rounded-lg` }>
                        <img className='scale-75'src={ window.location.origin + '/public/products/' + state.image } />
                    </div>
                </div>
                ...
                <div className='px-6'>
                    ...
                    <div className='flex justify-between gap-2 mb-4'>
                        <div className='flex gap-2'>
                            ...
                            <Stack spacing={ 1 }>
                                { user && state.owner === user.id ? 
                                    // <Rating name="product-rating" defaultValue={ parseFloat((sum/count).toFixed(1)) } precision={ 0.5 } readOnly/>
                                    ''
                                    : 
                                    <Rating name="product-rating" defaultValue={ state.ratings.length ? parseFloat(ratings) : parseFloat(0) } precision={ 0.5 } onChange={ isAuth ? (e) => addRatings(e) : () => redirectLogin() } />
                                }
                            </Stack>
                        </div>
                    ...
                    <div className='mt-4 mb-3 flex gap-2 justify-start'>
                        <p className='text-sm'>Available Stocks:</p>
                        <p className={ `${ state.stocks > 0 ? 'text-orange-400' : 'text-red-400' } text-sm font-semibold` }>{ state.stocks > 0 ? state.stocks : 'Out of Stock' }</p>
                    </div>
                    ...
                    <div className="fixed bottom-0 left-0 right-0 p-4 text-white text-center">
                        <div className="flex gap-6 justify-center items-center">
                            { state.stocks > 0 ? 
                                ...
                                :
                                <button className='bg-red-400 lg:hidden sticky top-0 z-40 font-semibold py-2 px-4 rounded-lg shadow-md' onClick={ () => { toast.error('Out of Stock !') } }>
                                    Out of Stock
                                </button>
                            }
                            <button
