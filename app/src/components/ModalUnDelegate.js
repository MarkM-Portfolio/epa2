...
const ModalUnDelegate = ({ show, onClose, children, unOtp }) => {
    ...
    const setUnDelegatePin = async (e) => {
        e.preventDefault()

        const CancelToken = axios.CancelToken
        const source = CancelToken.source()

        await axios.put(`/api/user/undelegate-pin/${ user.id }`, { unOtp: unOtp }, {
            headers: {
            ...
            <div className={ modalClasses }>
                ...
                <div className="mt-4">{ children }
                    <div className='flex justify-center text-white mt-4'>
                        <button onClick={ e => setUnDelegatePin(e) } className='bg-red-500 hover:bg-red-700 py-2 px-4 rounded'>Unlock</button>
                    </div>
                </div>
            </div>
        </div>
...
export default ModalUnDelegate
